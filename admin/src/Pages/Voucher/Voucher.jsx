...
const Voucher = () => {
    ...
    const handleAddVoucher = async () => {
        try {
            ...
            if (response.ok) {
                fetchVouchers();
                setNewVoucher({ name: "", description: "", expiry: "", discount: "", stock: "", image: "" });
            }
        } catch (error) {
    ...
    const handleEditVoucher = async () => {
        ...
        if (editingVoucher.image instanceof File) {
            ...
            const renamedImage = new File([editingVoucher.image], newFileName, { type: editingVoucher.image.type });
        ...
        try {
            const response = await fetch(`http://localhost:3000/api/coupons/${editingVoucher._id}`, {
                method: "PUT",
                headers: headers,
                body: requestBody
            });
    ...
    return (
        <div className="voucher-management">
            ...
            <div className="voucher-form">
                <h3>Thêm Voucher</h3>
                <input type="text" placeholder="Tên" value={newVoucher.name} onChange={(e) => setNewVoucher({ ...newVoucher, name: e.target.value })} required />
                <input type="text" placeholder="Mô tả" value={newVoucher.description} onChange={(e) => setNewVoucher({ ...newVoucher, description: e.target.value })} required />
                <input type="date" value={newVoucher.expiry} onChange={(e) => setNewVoucher({ ...newVoucher, expiry: e.target.value })} required />
                <input type="number" placeholder="Giảm giá (%)" value={newVoucher.discount} onChange={(e) => setNewVoucher({ ...newVoucher, discount: e.target.value })} required />
                <input type="number" placeholder="Số lượng" value={newVoucher.stock} onChange={(e) => setNewVoucher({ ...newVoucher, stock: e.target.value })} required />
                <input type="text" placeholder="URL hình ảnh" value={newVoucher.image} onChange={(e) => setNewVoucher({ ...newVoucher, image: e.target.value })} required />
            ...
            <div className="voucher-list">
                ...
                {vouchers.map((voucher) => (
                    <div key={voucher._id} className="voucher-item">
                        <img src={`http://localhost:3000/images/${voucher.image}`} alt={voucher.name} className="voucher-logo" />
                        <h4>{voucher.name}</h4>
                        <p>{voucher.description}</p>
                        <p>Giảm giá: {voucher.discount}%</p>
                        <p>Hết hạn: {new Date(voucher.expiry).toISOString().split("T")[0]}</p>
...
<input
    type="date"
    value={editingVoucher.expiry ? new Date(editingVoucher.expiry).toISOString().split("T")[0] : ""}
...
/>
        ...
        <input
            ...
            onChange={(e) => setEditingVoucher({ ...editingVoucher, stock: e.target.value })}
            required
        />
...
<input
            ...
            onChange={(e) => setEditingVoucher({ ...editingVoucher, image: e.target.value })}
            required
          />
