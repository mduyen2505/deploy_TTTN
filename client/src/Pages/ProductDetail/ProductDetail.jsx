...
const ProductDetail = () => {
  ...
  const orderProductMap =
    JSON.parse(localStorage.getItem("orderProductMap")) || {};
    ...
    const fetchReviews = async () => {
      try {
        const response = await axios.get(
          `https://deploytttn-production.up.railway.app/api/reviews/${id}`
        );
  ...
  const updateCartQuantity = async (productId, action) => {
    try {
      ...
      if (response.status === 200) {
        ...
        const updatedProduct = updatedCart.data.cart.find(
  ...
  const handleReviewSubmit = async () => {
    ...
    try {
      ...
      const response = await axios.post(
        "https://deploytttn-production.up.railway.app/api/reviews/",
        {
          productId: product._id,
          orderId,
          rating,
      ...
      <div className="product-detail-container">
        ...
        <div className="product-detail-info-box">
          ...
          <p className="product-detail-price">
            {product.discount > 0 ? (
              <>
                ...
                <span className="product-detail-discount-price">
                  {product.promotionPrice?.toLocaleString()}đ
                </span>
              </>
            ) : (
              `${product.price?.toLocaleString()}đ`
          ...
          <div className="product-detail-quantity">
            <button onClick={() => updateCartQuantity(product._id, "decrease")}>
              -
            </button>
            <span>{quantity}</span>
            <button onClick={() => updateCartQuantity(product._id, "increase")}>
              +
            </button>
          </div>
                    ...
                    <img
                      src={
                        review.userId.avatar
                          ? review.userId.avatar
                          : `https://ui-avatars.com/api/?name=${review.userId.username}`
                      }
                      alt="User Avatar"
